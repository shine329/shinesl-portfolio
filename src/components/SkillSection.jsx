...
const filteredSkills = skills.filter((skill)=> 
  ...
  return (
    <section id="skills" className="py-24 px-4 relative bg-secondaryu/30">
          ...
          {categories.map((category, key) => (
            <button key={key}
              ...
              className={cn("px-5  py-2 rounded-full transition-colors duration-300 capitalize",
                activeCategory === category? "bg-primary  text-forground" : "bg-secondary/70 text-foreground hover:bd-secondary"
              )}
            >
              {category}
            </button>
